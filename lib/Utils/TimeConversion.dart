...
String TimeConvert(time)
{
   ...
   else if(difference.inMinutes > 60 && difference.inSeconds > 60 && difference.inHours > 24 && difference.inDays > 7)
   {
     print(" Difference day is ${now.difference(timeToConvert).inDays}");
     return  (now.difference(timeToConvert).inDays/7).toString() + "week";
   }
   else
     {
...
String TimeConvertEvents(time)
{
  var x =  DateTime.parse(time).millisecondsSinceEpoch;
  DateTime timeToConvert = DateTime.fromMillisecondsSinceEpoch(x);
